...
$(function(){
    ...
    /*渲染*/
    $('.zm_history').html(template('historyTpl',{list:historyList}));
    $('.search_input').val('');
    ...
    /*3删除记录*/
    $('.zm_history').on('tap','.mui-icon',function(){
        /*1.获取索引*/
        ...
        localStorage.setItem('leTaoHistory',JSON.stringify(arr));
        /*5.重新渲染*/
        $('.zm_history').html(template('historyTpl',{list:arr}));
    });

    /*4清空记录*/
    $('.zm_history').on('tap','.fa',function(){
        /*清空数据*/
        localStorage.setItem('leTaoHistory','');
        /*重新渲染*/
        $('.zm_history').html(template('historyTpl',{list:[]}));
    });
